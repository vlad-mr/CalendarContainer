...
public class DatePickerCustomizationProvider {
    ...
    internal required init(withCell pickerCellView: DatePickerCell.Type?,
    ...
    func registerViews() {
        if let cellToRegister = cellView {
            pickerView?.register(cellToRegister.self, forCellWithReuseIdentifier: cellToRegister.reuseIdentifier)
            registeredCell = cellToRegister
        } else if let nibToRegister = cellNib {
            pickerView?.register(nibToRegister.getNib(), forCellWithReuseIdentifier: nibToRegister.reuseIdentifier)
            registeredCell = nibToRegister
        } else {
            let defaultNibToRegister = DayCell.self
            pickerView?.register(defaultNibToRegister.getNib(), forCellWithReuseIdentifier: defaultNibToRegister.reuseIdentifier)
            registeredCell = defaultNibToRegister
        }
    }

    public func dequeuePickerCell(forIndexPath indexPath: IndexPath) -> DatePickerCell? {
        guard let pickerCell = registeredCell else {
            return nil
        }
    ...
    func getWeekDayHeader(forDay _: Int) -> CustomizableDateHeader {
