...
class AnytimeCalLayoutAttributesGenerator: LayoutAttributesGenerator {
    ...
    var attributes = CalendarAttributes()
    ...
    required init(section width: CGFloat, height: CGFloat, slotSize: CalendarSlotSize, widthOfRowHeader: CGFloat, onlyFirstVerticalSeparator: Bool = false) {
        ...
        rowHeaderWidth = widthOfRowHeader
    ...
    func adjustItemsForOverlap(_ sectionItemAttributes: [UICollectionViewLayoutAttributes], inSection _: Int, sectionX: CGFloat) {
        ...
        for itemAttributes in sectionItemAttributes {
            ...
            overlappingItems.append(contentsOf: sectionItemAttributes.filter {
                $0 != itemAttributes ? itemFrame.intersects($0.frame) : false
            })
    ...
    func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for attributes in allLayoutAttributes where attributes.frame.intersects(rect) {
    ...
    func layoutAttributesForSupplementaryElements(ofKind kind: FullCalendarSupplementaryViewKind, at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        switch kind {
        case .hourLabel:
            return attributes.timeLabelsAttributes[indexPath]
        case .rowHeader:
            return attributes.timeHeaderAttributes[indexPath]
    ...
    func removeGeneratedAttributes(of type: AttributesType) {
        ...
        case .currentTimeIndicator:
            attributes.invalidateCurrentTimeIndicatorAttributes()
        case .event:
            attributes.invalidateEventAttributes()
        default:
