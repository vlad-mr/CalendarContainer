...
class EndReccurringViewController: UITableViewController {
    ...
    lazy private var currentFrequencyMode: EndReccurringMode = .never {
        didSet {
    ...
    lazy private var originalFrequencyMode: EndReccurringMode = .never
    lazy private var editedFrequencyMode: EndReccurringMode = .never {
        didSet {
            self.currentFrequencyMode = editedFrequencyMode
        }
    }
    
    lazy private var currentRule: RecurrenceRule = .init(frequency: .daily)
    ...
    lazy private var editedRule: RecurrenceRule = .init(frequency: .daily) {
        didSet {
    ...
    lazy private var datePicker: UIDatePicker = configure(UIDatePicker()) {
        $0.minimumDate = Date()
    ...
    lazy private var occorrencePicker: UIPickerView = configure(UIPickerView()) {
        $0.dataSource = self
        $0.delegate = self
    }
    
    // MARK: Table View Cells
    lazy private var neverCell: FrequencySelectionCell = configure(AnytimeNibs.frequencySelectionCell) {
        $0.configureCell(
            withTitle: "Never",
            shouldShowSeparator: true)
    }
    
    lazy private var onDateCell: FrequencySelectionCell = configure(AnytimeNibs.frequencySelectionCell) {
        $0.configureCell(
            withTitle: "On a date",
            shouldShowSeparator: true)
    }
    ...
    lazy private var occurrencesCell: FrequencySelectionCell = configure(AnytimeNibs.frequencySelectionCell) {
        $0.configureCell(
    ...
    private func updateOccurrencesCell() {
        let currentCellPickerData = viewModel.pickerData[self.occorrencePicker.selectedRow(inComponent: 0)]
