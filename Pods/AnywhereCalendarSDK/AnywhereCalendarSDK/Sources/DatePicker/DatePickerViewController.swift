...
open class DatePickerViewController: UIViewController, DatePickerView {
    ...
    override open func viewDidLoad() {
        ...
        self.view.frame = CGRect(x: 0, y: 0, width: self.view.frame.width, height: viewHeight)
        switch pickerConfig.viewConfiguration.pickerTitleMode {
        case .monthName:
            self.view.addSubview(monthTitle)
        ...
        self.view.addSubview(weekTitle)
    ...
    func updateHeaderView() {
        let originY: CGFloat = pickerConfig.viewConfiguration.pickerTitleMode != .none ? pickerDimensions.monthTitleHeight + 5 : 0
        let frame = CGRect(x: 0, y: originY, width: self.view.bounds.width, height: pickerDimensions.weekHeaderHeight)
    ...
    @objc func toggleMode() {
        ...
        self.pickerConfig.switchTo(newMode)
        self.handleMinMaxMode()
    }
    ...
    func resetView() {
        ...
        if let activeView = self.pageViewController.viewControllers?.first as? DatePickerCollectionViewController {
            ...
            if activeView.activeDate.compare(.isSameDay(activeDate)) {
                ...
                return
            } else if activeView.activeDate.isInSameMonthAs(activeDate) || self.pickerConfig.mode == .weekly {
        ...
        self.pageViewController.setViewControllers([monthPicker], direction: direction, animated: shouldAnimate, completion: nil)
    }
    
    func handleMinMaxMode() {
        
        let activeView = self.currentActivePageViewController!
        switch self.pickerConfig.mode {
        case .weekly:
            let weekOfMonth: CGFloat = CGFloat(self.lastSelectedDate?.weekOfMonth ?? self.activeDate.weekOfMonth)
            ...
            self.updateMonthPickerHeight(to: pickerDimensions.pickerDateHeight + 10)
        case .monthly:
            ...
            self.updateMonthPickerHeight(to: activeView.pickerHeight)
        }
        if self.pickerConfig.mode == .weekly && !(self.currentActivePageViewController?.model?.dates.contains(self.activeDate.dateAt(.startOfDay)) ?? true) {
            self.pageViewController.removeNonActivePages()
            self.resetView()
        } else {
            self.pageViewController.setViewControllers([activeView], direction: .forward, animated: false, completion: nil)
        }
